...
class Calendar extends React.Component {
	...
	render() {
		...
		return <ul className="calendar list-group">{days}</ul>;
	}
}
...
class CalendarDay extends React.Component {
	constructor(props) {
		...
		this.fetchCopySources = this.fetchCopySources.bind(this);
		this.copyFromPreviousDay = this.copyFromPreviousDay.bind(this);
	...
	fetchCopySources() {
		ApiClient.get(`/reports/copy_sources.json?date=${this.props.data.date}`)
			.then((response) => response.json())
			.then((data) => this.setState({ copySources: data }))
			.catch((error) =>
				console.log(`Failed to fetch copy sources: ${error.message}`),
			);
	}

	copyFromPreviousDay() {
		let ops = this.state.copySources.previous_day.operations;
		this.setState({ copiedOperations: ops, operationCount: ops.length });
	}

	copyFromLastWeek() {
		let ops = this.state.copySources.last_week.operations;
		this.setState({ copiedOperations: ops, operationCount: ops.length });
	}
	...
	render() {
		if (this.state.isEditing) {
			return (
				<li className={this.dayClassName()}>
					...
					<ReportForm
						...
						copiedOperations={this.state.copiedOperations}
					/>
					<CopyButtons
						copySources={this.state.copySources}
						copyFromPreviousDay={this.copyFromPreviousDay}
			...
			let operations = [];
			if (this.reportExist()) {
				operations = this.props.data.report.operations.map(
					(operation, index) => {
						return (
							<Operation
...
class CalendarDate extends React.Component {
	render() {
		...
		return (
			<div className="date">
				{parseInt(this.props.date.replace(/\d{4}-\d{2}-(\d{2})/, "$1"))}(
				{wdays[this.props.wday]})
			</div>
		);
	}
}
...
/**
 * 前日・先週の日報をコピーするボタンComponent
 */
class CopyButtons extends React.Component {
	render() {
		const { copySources, copyFromPreviousDay, copyFromLastWeek } = this.props;
		const prevDisabled =
			!copySources ||
			!copySources.previous_day ||
			!copySources.previous_day.available;
		const weekDisabled =
			!copySources ||
			!copySources.last_week ||
			!copySources.last_week.available;
		return (
...
class ReportForm extends React.Component {
	...
	componentWillMount() {
		if (this.props.isRegistered) {
			let displayed = [];
			for (
				let i = 0, imax = this.props.report.operations.length;
				i < imax;
				i++
			) {
				displayed.push(true);
	...
	validate(formdata) {
		...
		if (total !== 100) {
			this.setState({
				reportError:
					"稼働率の合計が100になっていません。現在" + total + "%です。",
			});
		}
	...
	render() {
		let action,
			...
			inputs = [],
			error = null;
		let copiedOps = this.props.copiedOperations;
		for (let i = 0; i < this.props.formLength; i++) {
			if (this.state.displayed[i] === false) {
				continue;
			}
			if (copiedOps && copiedOps[i]) {
				let copiedOp = copiedOps[i];
				operation = {
					id: null,
					project: { id: copiedOp.project_id, name: copiedOp.project_name },
					workload: copiedOp.workload,
				};
				inputs.push(
					<OperationForm
						operation={operation}
						...
						destroy={this.destroy}
						index={i}
						key={"copied-" + i + "-" + copiedOp.project_id}
					/>,
				);
			} else if (this.props.isRegistered && this.props.report.operations[i]) {
		...
		if (this.state.reportError) {
			error = (
				<span className="alert alert-danger">{this.state.reportError}</span>
			);
		}
		return (
