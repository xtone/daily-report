...
class RetirementProcessorModal extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			isOpen: false,
	...
	closeModal = () => {
		...
		if (status === "processing" || status === "pending") {
			if (
				!confirm(
					"処理中です。モーダルを閉じますか？処理はバックグラウンドで継続されます。",
				)
			) {
				return;
			}
	...
	handleSubmit = async (e) => {
		...
		const { lastWorkingDate, retirementDate } = this.state;
		const { userId } = this.props;

		if (!lastWorkingDate || !retirementDate) {
	...
	renderModal() {
		const { isOpen, lastWorkingDate, retirementDate, status, isSubmitting } =
			this.state;
...
export default RetirementProcessorModal;
