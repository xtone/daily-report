...
namespace :app do
  ...
  task :give_administrator_role_to, ['user_email'] => :environment do |_task, args|
  ...
  task :unsubmitted_notification_slack, ['channel'] => :environment do |_task, args|
    today = Date.today
    start_on = today.ago(60.days).to_datetime
    end_on = today.yesterday.to_datetime
    notifier = if args[:channel].present?
                 Slack::Notifier.new ENV.fetch('SLACK_WEBHOOK_URL', nil) do
